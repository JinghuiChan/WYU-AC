...
gcc unit-read-test-client.c -o unit-read-test-client `pkg-config --libs --cflags libmodbus`

### How to run

./unit-read-test-client tcp


modbus_read_registers  test:
ERROR Too many registers requested (255 > 125)
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
		...
		char strIP[21] = "127.0.0.1";
		if(argc > 2)
    {
        if (strcmp(argv[1], "tcp") == 0) {
            strcpy(strIP, argv[2]);
        } else {
            printf("Usage:\n  %s [tcp] [server IP] - Modbus client for unit testing\n\n", argv[0]);
    ...
    success = 0;
    const int Test_Count = 10000*50;
    while(success < Test_Count)
    {
    ...
    printf("1/2 modbus_write_register (%d): ",UT_REGISTERS_ADDRESS);
    //ASSERT_TRUE(rc == 1, "");
    ...
    for (i=0; i < nb_points; i++) {
       printf("192.168.0.%3d :    0x%04x \n",UT_REGISTERS_ADDRESS+i, tab_rp_registers[i]);
    }
